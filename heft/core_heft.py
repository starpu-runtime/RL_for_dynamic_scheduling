...
def cbar(ni, nj, agents, commcost):
    ...
    if n == 1:
        return 0
    npairs = n * (n - 1)
    return 1.0 * sum(commcost(ni, nj, a1, a2) for a1 in agents for a2 in agents if a1 != a2) / npairs
...
def ranku(ni, agents, succ, compcost, commcost):
    ...
    [1]. http://en.wikipedia.org/wiki/Heterogeneous_Earliest_Finish_Time
    """
    rank = partial(ranku, compcost=compcost, commcost=commcost, succ=succ, agents=agents)
...
def schedule(succ, agents, compcost, commcost):
    ...
    """
    rank = partial(ranku, agents=agents, succ=succ, compcost=compcost, commcost=commcost)
...
def recvs(job, jobson, prec, recv):
    ...
    if job not in prec:
        return []
    return [recv(jobson[p], jobson[job], p, job) for p in prec[job] if jobson[p] != jobson[job]]
...
def sends(job, jobson, succ, send):
    ...
    if job not in succ:
        return []
    return [send(jobson[job], jobson[s], job, s) for s in succ[job] if jobson[s] != jobson[job]]
...
def insert_recvs(order, jobson, prec, recv):
    ...
    recv_events = {e.job: [Event(r, e.start, e.start) for r in receives(e.job)] for e in order}
...
def insert_sends(order, jobson, succ, send):
    ...
    send_events = {e.job: [Event(s, e.end, e.end) for s in sends2(e.job)] for e in order}
