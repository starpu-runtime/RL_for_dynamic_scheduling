...
def test_cbar():
    ...
    assert cbar(1, 2, "Abc", commcost) == (6 + 6 + 3) / 3.0


def test_ranku():
    rank = partial(ranku, agents="abc", commcost=commcost, compcost=compcost, succ=dag)
    ...
    d = {3: ()}
    rank = partial(ranku, agents="abc", commcost=commcost, compcost=compcost, succ=d)
...
def test_schedule():
    ...
    orders
    assert orders == {
        a: [Event(4, 0, 4), Event(6, 4, 10), Event(7, 11, 18), Event(9, 18, 27)],
...
def test_makespan():
    assert makespan({"a": [Event(0, 0, 1), Event(1, 2, 3)], "b": [Event(2, 3, 4)]}) == 4
...
def test_insert_sendrecvs():
    ...
    assert all(e in neworders[agent] for agent in orders for e in orders[agent])
